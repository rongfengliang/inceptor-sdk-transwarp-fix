...
//        return confValue.toLowerCase().equalsIgnoreCase("true");
    ...
    private static HiveDate getHiveDate(String yyyy, String mm, String dd, String hourStr) {
        ...
        int MAX_MILLIS = 1;
        HiveDate d = null;
        if (yyyy.length() == 4 && mm.length() > 0 && mm.length() <= 2 && dd.length() <= 2 && dd.length() > 0) {
                    ...
                    if (hourStr == null) {
                        ...
                        if (firstColon > 0 && secondColon > 0 && secondColon < hourStr.length() - 1) {
                            ...
                            if (hh.length() == 2 && mM.length() == 2 && ss.length() == 2) {
                                ...
                                if (month >= 1 && month <= 12 && day >= 1 && day <= 31 && hour >= 0 & hour <= 23 && millis >= 0 && millis <= 59 && second >= 0 && second <= 59) {
                                    d = new HiveDate(year - 1900, month - 1, day, hour, millis, second);
                                }
                            }
